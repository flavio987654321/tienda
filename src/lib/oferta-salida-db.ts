...
import { leerOfertaSalida, codigoDeLaOferta, venceEnTexto } from "@/lib/oferta-salida";
...
export async function ofertaParaElMail(
  ...
  const vence = venceEnTexto(new Date(ahora.getTime() + o.horas * 3_600_000), ahora);
  ...
  if (o.tipo === "DESCUENTO") {
    ...
    /* El plazo se firma con la hora del envío: la cuenta corre desde que le
       llegó el mail, y el checkout respeta ese token si viene en el link. */
    const token = firmarOferta(principal.id, ahora.getTime());
    return {
