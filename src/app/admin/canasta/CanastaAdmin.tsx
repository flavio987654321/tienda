...
import { Upload, Loader2, Check, HeartHandshake, Plus, X } from "lucide-react";
import { useSavedNumberField } from "./useSavedNumberField";
...
function ProductRow({ product, onPriceSaved }: { product: Product; onPriceSaved: (id: string, price: number) => void }) {
      ...
      <div>
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded px-1.5 py-1 text-xs text-white"
          />
...
function AddProductCard({ campaignId, onAdded }: { campaignId: string; onAdded: (p: Product) => void }) {
  ...
  async function add() {
    ...
    try {
      const res = await fetch("/api/admin/canasta/products", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ campaignId, name: name.trim(), targetPrice }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-white/[0.02] p-3 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-gray-300">Nuevo alimento</span>
        <button type="button" onClick={() => { setOpen(false); setError(null); }} className="text-gray-500 hover:text-white">
