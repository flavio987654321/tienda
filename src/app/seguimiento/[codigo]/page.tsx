...
import {
  ...
  XCircle, Loader2, RefreshCw, Store, ArrowLeft,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      <nav className="bg-gray-950/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link href="/seguimiento" className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-white transition-colors">
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full px-6 py-10">
        ...
        {!loading && error && (
          <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
            ...
            <Link href="/seguimiento" className="mt-2 text-indigo-400 hover:text-indigo-300 text-sm font-semibold">
              ← Volver a buscar
            </Link>
          </div>
        )}
        ...
        {!loading && order && (
          <div className="space-y-5">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-indigo-400 text-sm font-semibold">{order.store.name}</span>
                </div>
                <h1 className="text-2xl font-black text-white">Pedido #{shortId}</h1>
            ...
            {/* Cancelled banner */}
            {isCancelled && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-2xl px-6 py-5 flex items-center gap-4">
                    ...
                    return (
                      <div key={step.key} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          ...
                          {i < STEPS.length - 1 && (
                            <div className={`w-0.5 my-1 ${i < currentIndex ? "bg-emerald-500/40" : "bg-white/8"}`} style={{ minHeight: 24 }} />
                          )}
                        </div>
            ...
            {order.trackingCode && (
              <div className="bg-indigo-600/10 border border-indigo-600/30 rounded-2xl px-6 py-5">
                <p className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-1">Número de seguimiento del envío</p>
                <p className="text-2xl font-black text-white font-mono tracking-wider">{order.trackingCode}</p>
              </div>
            )}
            ...
            <div className="grid md:grid-cols-2 gap-5">
                  ...
                  {order.items.map((item, i) => (
                    <div key={i} className="flex justify-between items-start gap-3">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-sm font-bold text-white">{fmt(item.price * item.quantity)}</p>
                ...
                <div className="mt-4 pt-4 border-t border-white/5 space-y-1.5">
                  {order.subtotal !== order.total && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-gray-400">{fmt(order.subtotal)}</span>
                    </div>
                  )}
                  ...
                  {order.shippingCost > 0 && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-gray-400">{fmt(order.shippingCost)}</span>
                    </div>
                  )}
                  <div className="flex justify-between pt-1">
                    <span className="text-sm font-bold text-white">Total</span>
                    <span className="text-lg font-black text-white">{fmt(order.total)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-900 border border-white/10 rounded-2xl p-5">
                  ...
                  <p className="text-sm font-semibold text-white">{order.shippingMethod || "A coordinar con el vendedor"}</p>
                  {order.shippingCost === 0
                    ? <p className="text-xs text-emerald-400 mt-0.5">Sin costo de envío</p>
