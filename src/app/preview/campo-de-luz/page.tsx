...
function Tarjeta({ p, indice }: { p: (typeof PRODUCTOS)[number]; indice: number }) {
  return (
    <PiezaQueLlega indice={indice}>
      <Inclinable grados={5} style={{ borderRadius: 18 }}>
        <article style={{ ...vidrio("oscuro"), borderRadius: 18, overflow: "hidden" }}>
          <div style={{ aspectRatio: "3/4", overflow: "hidden", background: "#0e0f1a", borderRadius: "18px 18px 0 0" }}>
    ...
    <div style={{ background: BASE, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "70px 26px 120px", maxWidth: 1180, margin: "0 auto" }}>
        ...
        <p style={{ margin: "0 0 34px", fontSize: 13, color: MID }}>
          La ficha ya vive en Aurora, con el motor real de talles, stock y carrito.
        </p>
        <GrillaProfunda min={230}>
          {PRODUCTOS.map((p, i) => (
            <Tarjeta key={p.id} p={p} indice={i} />
          ))}
        </GrillaProfunda>
      </div>
