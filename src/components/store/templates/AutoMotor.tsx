...
export default function AutoMotor() {
  ...
  const heroOverlayOp   = heroOv?.overlayOpacity ?? 0.72;
  const heroIsLight     = heroOverlayType === "light";
  const heroText        = heroIsLight ? "#111111" : "#ffffff";
  const heroMid         = heroIsLight ? "rgba(0,0,0,0.55)" : "rgba(255,255,255,0.55)";
  const heroNavMid      = heroIsLight ? "#555555" : "rgba(255,255,255,0.65)";
  const heroNavBorder   = heroIsLight ? "rgba(0,0,0,0.25)" : "rgba(255,255,255,0.25)";
  ...
  const catalogoBg  = sc["bgCatalogo"]  ?? "#111111";
  ...
  const serviciosBg = sc["bgServicios"] ?? "#0d0d0d";
  ...
  const nosotrosBg  = sc["bgNosotros"]  ?? "#0a0a0a";
  ...
  const contactoBg  = sc["bgContacto"]  ?? "#0d0d0d";
  ...
  const footerBg    = sc["bgFooter"]    ?? "#080808";
  ...
  const [activeCond, setActiveCond] = useState("Todas");
  ...
  const condList = useMemo(() => {
    const vals = products.map(p => attr(p, "condicion") || attr(p, "Condición")).filter(Boolean);
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCond !== "Todas") r = r.filter(p =>
      (attr(p, "condicion") || attr(p, "Condición")) === activeCond);
    return r;
  }, [products, activeCat, activeCond]);
  ...
  return (
    <div style={{ background: "#111111", color: "#ffffff",
      ...
      <style>{`
        ...
        .am-filters { flex-direction:column; gap:8px }
        @media(min-width:560px){ .am-filters { flex-direction:row; gap:12px } }
      ...
      {showAnn && (
        <div style={{ position: isPreview ? "sticky" : "fixed", top:0,
          ...
          background: "#000", borderBottom: `1px solid ${accent}44`,
          display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontSize:11, fontWeight:600, color:accent, letterSpacing:2 }}>
            {annMessages[annIdx]}
          </span>
              ...
              {annMessages.map((_, i) => (
                <button key={i} onClick={() => setAnnIdx(i)}
                  style={{ width: i===annIdx ? 14 : 5, height:3, border:"none", borderRadius:2,
                    background: i===annIdx ? accent : "rgba(255,255,255,0.2)", cursor:"pointer", padding:0, transition:"all 0.3s" }} />
              ))}
            </div>
          )}
          <button onClick={() => setAnnVisible(false)}
            style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)",
              background:"none", border:"none", color:"#fff", cursor:"pointer", fontSize:16, opacity:0.5 }}>×</button>
        </div>
      )}
      ...
      <nav style={{ position: isPreview ? "sticky" : "fixed",
        ...
        <div style={{ maxWidth:1200, margin:"0 auto", height:NAV_H,
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6,
            fontWeight:900, fontSize:15, letterSpacing:4, textTransform:"uppercase",
            color: scrolled ? accent : heroText, transition:"color 0.35s" }}>
            ...
            {[["Catálogo","catálogo"],["Servicios","servicios"],["Nosotros","nosotros"],["Contacto","contacto"]].map(([lbl,id]) => (
              <button key={id} onClick={() => smoothScrollTo(id)}
                style={{ background:"none", border:"none", cursor:"pointer", fontSize:11,
                  ...
                  color: scrolled ? "rgba(255,255,255,0.5)" : heroNavMid }}
                onMouseEnter={e => (e.currentTarget.style.color=accent)}
                onMouseLeave={e => (e.currentTarget.style.color= scrolled ? "rgba(255,255,255,0.5)" : heroNavMid)}>
                {lbl}
              </button>
            ))}
            ...
            {pushBell && config?.showPushBell && !isPreview && (
              <button onClick={pushBell.openDrawer}
                style={{ position:"relative", background:"none", border:"none", cursor:"pointer", padding:4,
                  ...
                  color: scrolled ? "rgba(255,255,255,0.5)" : heroText, transition:"color 0.35s" }}>
                ...
                {pushBell.hasNew && <span style={{ position:"absolute", top:2, right:2, width:10, height:10,
                  background:"#ef4444", borderRadius:"50%", border:"2px solid #0a0a0a" }} />}
              </button>
            )}
        ...
        {menuOpen && (
          <div style={{ background:"#0a0a0a", borderTop:"1px solid rgba(255,255,255,0.07)", padding:"8px 28px 20px" }}>
            {[["Catálogo","catálogo"],["Servicios","servicios"],["Nosotros","nosotros"],["Contacto","contacto"]].map(([lbl,id]) => (
      ...
      {/* ── STATS STRIP ────────────────────────────────────────── */}
      <div style={{ background:"#000", borderTop:`1px solid ${accent}22`,
        borderBottom:`1px solid ${accent}22`, padding:"0 28px" }}>
      ...
      <section id="catálogo" style={{ padding:"72px 28px", position:"relative",
        ...
        <div style={{ position:"relative", zIndex:1, maxWidth:1200, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between",
            ...
            <Link href={`/tienda/${config?.slug ?? ""}/vehiculos${isPreview ? "?from=editor" : ""}`}
              style={{ fontSize:12, color:accent, fontWeight:700, letterSpacing:0.5, textDecoration:"none" }}>
              Ver todos →
            </Link>
          </div>

          {/* Filters */}
          {(categoryList.length > 1 || condList.length > 1) && (
            <div className="am-filters" style={{ display:"flex", marginBottom:24 }}>
              {categoryList.length > 1 && (
                <select value={activeCat} onChange={e => setActiveCat(e.target.value)}
                  style={{ flex:1, background: catText==="#ffffff" ? "#1e1e1e" : "#fff",
                    color:catText, border:`1px solid ${catText==="#ffffff" ? "#2a2a2a" : "#e0e0e0"}`,
              ...
              {condList.length > 1 && (
                <select value={activeCond} onChange={e => setActiveCond(e.target.value)}
                  style={{ flex:1, background: catText==="#ffffff" ? "#1e1e1e" : "#fff",
                    color:catText, border:`1px solid ${catText==="#ffffff" ? "#2a2a2a" : "#e0e0e0"}`,
                    padding:"10px 14px", fontSize:12, fontWeight:600, cursor:"pointer",
                    outline:"none", letterSpacing:0.5 }}>
          ...
          {loadingProducts ? (
            ...
            <div style={{ textAlign:"center", padding:"60px 0",
              border:`1px dashed ${catText==="#ffffff" ? "#333" : "#ddd"}` }}>
              <p style={{ margin:0, color:catMid, fontSize:14 }}>
                {products.length===0 ? "Aún no hay vehículos publicados." : "Sin resultados para este filtro."}
              </p>
            </div>
          )}
            ...
            <div style={{ textAlign:"center", marginTop:40 }}>
              <Link href={`/tienda/${config?.slug ?? ""}/vehiculos${isPreview ? "?from=editor" : ""}`}
                style={{ display:"inline-flex", alignItems:"center", gap:10,
                  ...
                  textDecoration:"none", padding:"15px 44px",
                  fontWeight:800, fontSize:11, letterSpacing:2, textTransform:"uppercase" }}>
                Ver los {filtered.length} vehículos →
              </Link>
            </div>
          )}
        </div>
      ...
      <section id="servicios" style={{ padding:"80px 28px", position:"relative",
        ...
        <div style={{ position:"relative", zIndex:1, maxWidth:1200, margin:"0 auto" }}>
          ...
          <div className="am-svc" style={{ display:"grid", gap:2 }}>
            {[
              ...
              <div key={i}
                style={{ padding:"36px 28px",
                  background: svcText==="#ffffff" ? "#1a1a1a" : "#f8f8f8",
                  border:`1px solid ${svcText==="#ffffff" ? "#222" : "#e8e8e8"}`,
                  borderBottom:`3px solid transparent`,
                  transition:"border-bottom-color 0.2s" }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderBottomColor=accent; }}
                ...
                <p style={{ margin:"0 0 14px", fontSize:42, fontWeight:900, color:accent,
                  lineHeight:1, letterSpacing:-2, fontVariantNumeric:"tabular-nums" }}>{s.n}</p>
          ...
          {whatsapp.enabled && whatsapp.number && (
            <a href={`https://wa.me/${whatsapp.number.replace(/\D/g,"")}${whatsapp.message?"?text="+encodeURIComponent(whatsapp.message):""}`}
              ...
              style={{ display:"inline-flex", alignItems:"center", gap:12, background:"#25d366",
                ...
                fontWeight:900, fontSize:15, boxShadow:"0 12px 40px rgba(37,211,102,0.25)" }}>
              <WaIcon size={22} />
      ...
      <footer style={{ position:"relative", padding:"32px 28px", textAlign:"center",
        ...secBg(footerImg, footerBg), borderTop:"1px solid rgba(255,255,255,0.05)" }}>
