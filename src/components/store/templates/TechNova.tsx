      ...
      <div style={{ padding:"14px 16px" }}>
        ...
        {promo.hasPriceDrop ? (
          <>
            ...
            <p style={{ margin:"2px 0 0", fontSize:12, color:"#9a9ab0", textDecoration:"line-through" }}>{fmtPrice(promo.originalPrice, currency)}</p>
          </>
        ) : (
          <>
          ...
          <div className="tn-nav-links" style={{ gap:30, alignItems:"center" }}>
            <div className="tn-mega-wrap">
              ...
              <div className="tn-megamenu" style={{ position:"absolute", top:"100%", left:0, right:0,
                background:"#ffffff", borderTop:"1px solid #ececf5", boxShadow:"0 24px 50px rgba(15,15,26,0.12)", padding:"28px 24px", zIndex:CAPAS.nav }}>
              ...
              {userDropdownOpen && (
                <div style={{ position:"absolute", top:"calc(100% + 10px)", right:0, background:"#fff", border:`1px solid ${navBorder}`, minWidth:190, zIndex:CAPAS.flotante, boxShadow:"0 8px 28px rgba(0,0,0,0.18)", borderRadius:10, overflow:"hidden" }}>
                  {cargando ? (<p style={{ padding:"14px 16px", margin:0, fontSize:12, opacity:0.55 }}>Cargando…</p>) : logueado ? (
                    <>
                      <p style={{ padding:"10px 16px 4px", fontSize:11, color:"#9a9ab0", margin:0, fontWeight:600, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{nombreMostrado}</p>
