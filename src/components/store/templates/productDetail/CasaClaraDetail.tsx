...
export default function CasaClaraDetail({ view }: { view: ProductDetailViewProps }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.pageBg, fontFamily: theme.font, color: theme.text }}>
      <nav style={{ borderBottom: `1px solid ${theme.cardBorder}`, padding: "0 24px", position: "sticky", top: 0, background: theme.pageBg, zIndex: CAPAS.navFicha }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Link href={homeHref} style={{ fontWeight: 600, fontSize: 17, color: theme.text, textDecoration: "none", letterSpacing: 2, textTransform: "uppercase" }}>{storeName}</Link>
