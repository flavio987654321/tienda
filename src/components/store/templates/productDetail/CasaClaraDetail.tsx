...
export default function CasaClaraDetail({ view }: { view: ProductDetailViewProps }) {
    ...
    <div style={{ minHeight: "100vh", background: theme.pageBg, fontFamily: theme.font, color: theme.text }}>
      <nav style={{ borderBottom: `1px solid ${theme.cardBorder}`, padding: "0 24px", position: "sticky", top: 0, background: theme.pageBg, zIndex: 50 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Link href={`/tienda/${slug}`} style={{ fontWeight: 600, fontSize: 17, color: theme.text, textDecoration: "none", letterSpacing: 2, textTransform: "uppercase" }}>{storeName}</Link>
      ...
      {toastMsg && (
        <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: "#0f172a", color: "#fff", fontSize: 13, padding: "10px 18px", borderRadius: 100, zIndex: 60 }}>
          {toastMsg}
        </div>
      )}
    </div>
