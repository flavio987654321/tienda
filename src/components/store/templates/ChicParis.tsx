  ...
  return (
    <div style={{ fontFamily: "'Inter','Helvetica Neue',Arial,sans-serif", background: "#fff", color: "#111", minHeight: "100vh" }}>
        ...
        <div style={{ padding: "0 32px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {!isMobile && <nav style={{ display: "flex", gap: 24, alignItems: "center" }}>
            ...
            {categoryList.length > 0 && <div style={{ position: "relative" }}
              ...
              {hoveredNavCat && (
                ...
                <div style={{ position: "absolute", top: "calc(100% + 12px)", left: 0, background: "#fff", border: "1px solid #e8e8e8", zIndex: CAPAS.panel, padding: "24px 32px", boxShadow: "0 12px 40px rgba(0,0,0,0.12)", display: "flex", gap: 40 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div ref={userDropdownRef} style={{ position: "relative" }}>
              ...
              {userDropdownOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #e8e8e8", borderRadius: 10, minWidth: 180, boxShadow: "0 8px 24px rgba(0,0,0,0.1)", zIndex: CAPAS.modalTemplateAlto, overflow: "hidden" }}>
      ...
      {isMobile && mobileMenuOpen && (
        <div style={{ position: isPreview ? "sticky" : "fixed", top: isPreview ? 0 : 68 + (showAnnouncement ? PROMO_BAR_H : 0), left: 0, right: 0, bottom: 0, background: "#fff", zIndex: CAPAS.barraAccion, overflowY: "auto", overscrollBehavior: "contain" }}>
          {/* Categorías — acordeón */}
              ...
              <div key={titleField} style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: "center", textAlign: isMobile ? "center" : "left", gap: isMobile ? 8 : 14, flex: isMobile ? undefined : "1 1 200px" }}>
                <div style={{ color: ACC, flexShrink: 0, position: "relative" }}>
                  ...
                  {/* ── Cambiar ícono ────────────────────────────────────────
                      ...
                      `zIndex:CAPAS.nav` —las flechas de orden van abajo y al centro— y
                  ...
                  {editMode && (
                    <button
                      ...
                      style={{ position: "absolute", top: -12, left: -12, zIndex: CAPAS.navMenu, width: 20, height: 20, background: "rgba(99,102,241,0.92)", border: "1.5px solid rgba(255,255,255,0.5)", borderRadius: 5, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, lineHeight: 1, boxShadow: "0 2px 6px rgba(0,0,0,0.3)", opacity: 0.8, transition: "opacity 0.15s" }}
...
{r.product?.name && (
  <p style={{ fontSize: 11, color: "#6e6e6e", margin: 0, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{r.product.name}</p>
      ...
      {/* ── SEARCH OVERLAY ── */}
      {searchOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: CAPAS.galeriaTemplate, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "80px 20px 20px", backdropFilter: "blur(4px)" }}
      ...
      {/* ── TOAST ── */}
      {toastMsg && (
        <div style={{ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", background: "#111", color: "#fff", padding: "12px 24px", borderRadius: 4, fontSize: 13, fontWeight: 600, zIndex: CAPAS.critico, animation: "cp-toast 0.3s ease", boxShadow: "0 8px 24px rgba(0,0,0,0.25)", maxWidth:"calc(100vw - 32px)", textAlign:"center" }}>
          {toastMsg}
        </div>
      )}
    </div>
