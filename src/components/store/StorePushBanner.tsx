...
export default function StorePushBanner({ storeName }: { storeName: string }) {
  ...
  const { campaigns, loadingCampaigns, drawerOpen, closeDrawer, followState } = bell;
  const isFollowing = followState === "following";

  // Acá vivía un banner "Activá las notificaciones" fijo abajo. Pedía lo mismo
  // que el botón de seguir (mismo `needsPushActivation`, mismo
  // `activatePushOnDevice`) y caía en las mismas coordenadas que el cartel de
  // instalar la PWA, que lo tapaba entero. Ese flujo lo maneja StoreFollowButton.
